...
# Current theoretical convention:
# rho_obs is used only to identify saturated measured traces.
# The load-dependent model itself uses rho0 = lambda*S0/c, where S0 and beta
# are the hyperbolic fit parameters exported by fit_service_time.py.
...
# ── Core DES engine ──────────────────────────────────────────────────────────
def run_des(arrivals_ms, mean_svc_ms, cv, c, beta=0.0, rho0=0.0, seed=0):
    """
    ...
    if beta > 0 and rho0 > 0:
        s_eff = mean_svc_ms / max(0.01, 1.0 - beta * min(rho0, 0.95))
    else:
        s_eff = mean_svc_ms
...
def main():
    ...
    for row_idx, (tag, (folder, c, prefix)) in enumerate(SERVER_SPECS.items()):
        ...
        for tf in trace_files:
            ...
            # Observed rho is used only to skip saturated traces. The model
            # correction uses nominal rho0 = lambda*S0/c.
            rate_row = sum_df[sum_df["rate_rps"] == rate]
            if rate_row.empty:
                continue
            rho0 = rate * s0_ms / (c * 1000.0)
            ...
            for seed in range(N_RUNS):
                std_resp.extend(run_des(arrivals, s0_ms, cv_est, c,
                                        beta=0.0, rho0=rho0, seed=seed))
                ld_resp.extend( run_des(arrivals, s0_ms, cv_est, c,
                                        beta=beta,  rho0=rho0, seed=seed))
            ...
            s_eff = s0_ms / max(0.01, 1.0 - beta * min(rho0, 0.95))
            print(f"  rate={rate:4d} rps  rho_obs={rho_obs:.3f}  rho0={rho0:.3f}  "
                  f"S_eff={s_eff:.2f}ms  "
        ...
        for col, prate in enumerate(plot_rates):
            ...
            ax.set_title(
                f"{label}  {prate} rps  (rho_obs={rho_obs:.2f}, rho0={rho0:.2f})\n"
    ...
    plot_out = os.path.join(FIG_DIR, "des_ld_cdf.png")
    ...
    if results:
        ...
        csv_out = os.path.join(TABLE_DIR, "des_ld_results.csv")
