"""
run_sweeps.py - Orchestrates server load sweeps for the capacity thesis.

Runs each (server, rate) pair sequentially:
  1. Clears the live CSV log written by the container.
  2. Runs the appropriate Poisson load generator.
  3. Copies the result into data/experiments/<server_folder>/.

Usage:
  python analysis/orchestration/run_sweeps.py --servers node_dsp go_sqlite
...
LOGS_DIR = os.path.join(ROOT, "data", "live_logs")
EXP_BASE = os.path.join(ROOT, "data", "experiments")

DSP_LOAD = os.path.join(ROOT, "analysis", "load", "dsp_aes_load.py")
SQLITE_LOAD = os.path.join(ROOT, "analysis", "load", "sqlite_load.py")
...
def run_sweep(server: str, rate: float) -> bool:
    ...
    print(f"\n{'=' * 60}")
    print(f"  {server}  {rate} rps  ->  data/experiments/{exp_folder}/")
    print(f"{'=' * 60}")
    ...
    for server in servers_to_run:
        ...
        for rate in SWEEP_RATES[server]:
            ...
            if os.path.exists(dest) and os.path.getsize(dest) > 200:
                ...
                continue
            run_sweep(server, rate)
            done += 1
            elapsed = time.time() - t_start
            remaining = (elapsed / done) * (total - done) if done else 0
            print(f"  Progress: {done}/{total}  elapsed={elapsed / 60:.1f}min  remaining~{remaining / 60:.1f}min")
