...
python dsp_aes_load.py --url http://localhost:8085/process --rate 50 --duration 90

# 3. Run DES on the collected trace
python "logs and des/single_server_des.py" \
  --input experiments/python_dsp_1c/<trace_file>.csv \
  --mode replay --output des_out.csv

# 4. Re-run all DES modes and regenerate summary + CDF
python run_des_all.py --servers python_dsp_1c
python plot_all_cdfs.py python_dsp_1c
```
