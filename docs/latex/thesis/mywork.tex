...
\begin{itemize}
    ...
    \item calibrates interpretable service-response models from real traces,
    \item injects those calibrated parameters into DES simulations,
...
This thesis is the first to assemble these components into a coherent, reproducible pipeline for both teaching and research. By grounding classical capacity-planning theory in real measurements and trace-driven DES, it addresses the pedagogical gap in COMP9334 and contributes a novel methodology to the performance-engineering literature. The final result is more mechanistic than the original ML plan: ML remains important for future arrival-rate forecasting, while service-time modelling is handled by interpretable load-dependent curve fits.
