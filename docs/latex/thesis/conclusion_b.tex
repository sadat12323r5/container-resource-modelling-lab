...
The final interpretation is more cautious than the initial framing above: the experiments broadly support the structural prediction, but not every server is positive evidence for pure CPU Processor Sharing. Some servers are controls with near-constant service time, Java is affected by JIT warmup, and SQLite points toward a database-lock/tandem-queue explanation. This distinction is important because the thesis contribution is strongest when it identifies \emph{which} model assumption fails, rather than forcing every server into the same explanation.
...
This thesis extended the lightweight Docker-based capacity-planning platform established earlier in the project into a comprehensive multi-server experimental and modelling framework. The work produced the following concrete contributions.
...
The central empirical finding is that the M/G/$c$ constant-service-time assumption is violated for several CPU-bound server implementations in the study. Mean service time grows with nominal utilisation and can be modelled by the hyperbolic form:

\[
E[S](\rho_0) = \frac{S_0}{1 - \beta\rho_0},
\]

where $\rho_0=\lambda S_0/c$ and $\beta \in [0, 1)$ quantifies the degree of OS-level Processor Sharing caused by kernel CPU time-slicing among concurrent request handlers. The parameter $\beta$ provides an interpretable characterisation of how strongly wall-clock service time changes under load. The result is deliberately not universal: Python DSP~(1c), Node DSP~(3c), and Java DSP~(3c) behave approximately as constant-service controls over the measured range, while Apache DSP, Go lognormal, Node DSP~(1c), and Python DSP~(3c) provide the main positive cases.

\subsection{M0/M1/M2 DES, Histograms, and ANOVA}

A load-dependent variant of the M/G/$c$ discrete-event simulator was implemented and validated against measured traces using Kolmogorov--Smirnov distance. The final validation compares four CDFs: the measured response-time CDF (CDF\_REF), the constant-service DES (CDF\_M0), the linear-service DES (CDF\_M1), and the hyperbolic-service DES (CDF\_M2). The important finding is nuanced: M2 does not dominate M1 at every point, but it clearly improves on M0 in the workloads where the constant-service assumption fails. For example, Python DSP~(3c) at 200~rps improves from KS $0.2943$ under M0 to $0.1153$ under M2, Apache DSP~(1c) at 75~rps improves from $0.4828$ to $0.3671$, and Go lognormal~(1c) at 200~rps improves from $0.1815$ to $0.0925$.

This simulation evidence is supported by two additional checks. First, per-rate service-time histograms show that affected servers develop shifted and heavier-tailed service-time distributions as arrival rate increases. Second, one-way ANOVA on $\log(\texttt{service\_ms})$ rejects the constant-service hypothesis for the main affected workloads, with meaningful effect sizes for Apache DSP~(1c) ($\eta^2=0.3074$), Node DSP~(1c) ($\eta^2=0.1042$), Go lognormal~(1c) ($\eta^2=0.0339$), and Python DSP~(3c) ($\eta^2=0.0268$). Because the traces are large, the thesis interprets ANOVA using effect size as well as $p$-value.

\subsection{Exploratory Priority and Feedback Validation}

Two explicit scheduling alternatives were also implemented and tested: a preemptive-resume priority M/G/1 model with synthetic short high-priority work interrupting DSP jobs, and a feedback-priority model where DSP jobs receive a first high-priority quantum before unfinished work returns to a lower-priority queue. These models were motivated by the concern that M2 should not be interpreted as a literal Processor-Sharing queue nested inside an FCFS queue.

The exploratory result is conservative. On selected high-mismatch traces, the priority models improved over plain FCFS M0 in some cases, but did not outperform the existing load-dependent M2 correction overall. Average KS distance was $0.4199$ for FCFS M0, $0.3727$ for M2, $0.4054$ for preemptive-resume priority, and $0.4215$ for feedback priority. This strengthens the thesis claim by showing that a more complex scheduling model was considered and tested, but that the current traces still support the simpler load-dependent service-time correction. It also clarifies what follow-up work would need: new traces with real priority classes or feedback visits, rather than attempting to infer hidden priority structure from one-class DSP traces.

\subsection{Interactive Visualisation and Live Load Tool}

A browser-based visualiser (\texttt{visualiser.html}) was built to make the modelling framework accessible to students and researchers. It supports interactive DES parameter tuning, CSV trace upload with CDF overlay and live KS distance display, Gantt timeline replay of trace data, and a WebSocket live-load backend (\texttt{vis\_server.py}) that can fire real HTTP requests at any running Docker server and stream results to the browser. This toolchain satisfies the project's pedagogical goal: students can now observe, simulate, and validate capacity-planning behaviour all in one environment running on a personal computer.

%-------------------------------------------------------------
\section{Limitations}

\paragraph{Fixed-point utilisation.}
The current M0/M1/M2 validation evaluates each trace at its observed arrival rate and uses $\rho_0=\lambda S_0/c$ to choose the fitted service mean. This works for comparing against traces that were collected at known operating points. A true capacity forecaster, however, must solve the self-consistency problem at an unobserved future arrival rate, where the inflated service time changes utilisation and utilisation changes service time. That requires a fixed-point solver for $\rho^* = \lambda E[S](\rho^*)/(c \cdot 1000)$.
...
The supervisor has suggested extending the modelling framework to more complex setups that are harder to model analytically and where novel queueing contributions are possible. The exploratory priority validation shows that these directions should be pursued only with newly instrumented workloads that expose the relevant structure directly. Three candidate directions are:
\begin{itemize}
    \item \emph{Two-class priority M$^{[2]}$/G/1}: a system where background batch requests compete with interactive foreground requests under preemptive or non-preemptive priority. The next experiment should add explicit request classes (for example \texttt{/short} and \texttt{/dsp}) and log the class of every request, so that the priority formula can be validated against class-specific response CDFs rather than synthetic hidden traffic.
...
The data required for this already exists: the per-server summary CSVs contain \texttt{rate\_rps} time series, and Prometheus stores scrape-interval $\lambda$ traces for the duration of every load-test experiment. Implementing ARIMA on this data and feeding predictions into the load-dependent DES would complete the hybrid ML--DES pipeline originally planned for the project and represent a practical, deployable capacity-planning tool.
