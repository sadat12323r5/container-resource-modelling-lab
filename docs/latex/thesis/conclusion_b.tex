...
The final Thesis~B interpretation is more cautious than the initial framing above: the experiments broadly support the structural prediction, but not every server is positive evidence for pure CPU Processor Sharing. Some servers are controls with near-constant service time, Java is affected by JIT warmup, and SQLite points toward a database-lock/tandem-queue explanation. This distinction is important because the thesis contribution is strongest when it identifies \emph{which} model assumption fails, rather than forcing every server into the same explanation.
