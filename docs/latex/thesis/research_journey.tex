...
This observation motivated fitting the hyperbolic model $E[S](\rho_0) = S_0 / (1 - \beta\rho_0)$, which borrows the saturation shape of the PS mean response-time formula while using nominal utilisation $\rho_0=\lambda S_0/c$. The coefficient $\beta$ measures how strongly the server's wall-clock service time is affected by load. The model was fitted using nonlinear least squares with AIC-based model selection against two competing hypotheses (constant and linear). The later results were more nuanced than the first interpretation: AIC often selected the linear model over the measured range, but the hyperbolic model remained the mechanistic candidate because it has the correct PS-like interpretation near saturation.

\textbf{This became the central contribution of the thesis.} The standard M/G/$c$ model, calibrated with the correct parameters and validated against real traces, can underestimate response times at moderate to high utilisation when the service-time distribution itself changes with load. A single additional parameter $\beta$, estimable from a small number of load-test traces, gives an interpretable way to test and correct that violation.
...
\subsection{D-16: Supervisor Follow-Up Forced a Stronger Test}

After the first report of the $\beta$ model, the supervisor raised an important question: fitting service time is not enough. If M2 is a better service-time model, does it actually produce a better response-time CDF in the DES? This led to a new comparison among CDF\_REF, CDF\_M0, CDF\_M1, and CDF\_M2 for each stable trace.

This follow-up changed the interpretation again. The result was not that M2 always wins. In several finite-range cases, the linear M1 model is as good as or better than M2. However, the comparison produced clear cases where M0 fails and M2 improves the response CDF substantially: Python DSP~(3c) at 200~rps, Apache DSP~(1c) at 75~rps, and Go lognormal~(1c) at 200~rps. This made the claim more defensible. The innovation is not ``M2 is best everywhere''; it is that the M0 constant-service assumption is empirically false for specific workloads and that a PS-motivated correction helps exactly where that assumption fails.

The same follow-up also motivated service-time histograms by arrival rate and an ANOVA test on $\log(\texttt{service\_ms})$. These added statistical evidence to the visual and simulation evidence. The histograms show right-skewed distributions whose centre and tail shift with load, and the ANOVA results reject service-time independence for the main affected workloads. This became the final evidence chain: measured distributions, DES CDF comparisons, and formal statistical tests all point to the same load-dependence finding.

%-------------------------------------------------------------
\section{How the Research Direction Changed}
\label{sec:journey-pivot}

The original plan, as stated in the earlier project plan, centred on a hybrid ML--DES pipeline: machine-learning models (KDE, GMM, gradient-boosted regressors) would learn empirical service-time distributions as a function of CPU quota and arrival rate, and ARIMA or LSTM models would forecast arrival rates from monitoring data. These ML components would supply the DES with distributional parameters.
