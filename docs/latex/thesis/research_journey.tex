...
The same follow-up also motivated service-time histograms by arrival rate and an ANOVA test on $\log(\texttt{service\_ms})$. These added statistical evidence to the visual and simulation evidence. The histograms show right-skewed distributions whose centre and tail shift with load, and the ANOVA results reject service-time independence for the main affected workloads. This became the final Thesis~B evidence chain: measured distributions, DES CDF comparisons, and formal statistical tests all point to the same load-dependence finding.
