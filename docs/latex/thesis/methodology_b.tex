...
The core DES engine (\texttt{run\_des} in \texttt{des\_load\_dependent.py}) operates in \emph{replay mode}: it consumes the exact arrival sequence from an observed trace and produces a simulated response-time distribution by sampling service times from the fitted model.
...
A two-pointer $O(n + m)$ implementation operating on pre-sorted arrays is used. Lower KS distance indicates better agreement between the simulated and observed response-time CDFs. The improvement $\Delta = d_\text{KS}(\text{std}) - d_\text{KS}(\text{LD})$ is the primary figure of merit for evaluating the load-dependent model.
