...
This chapter describes the experimental platform used to collect the traces analysed in the rest of the thesis. The platform began as a single Dockerised microservice for observing basic capacity-planning phenomena: growing mean latency, queue formation, and CPU saturation on commodity hardware. It was then expanded into a systematic multi-server experimental programme. Nine distinct server implementations across five languages and two concurrency configurations are subjected to open-loop load testing at a range of arrival rates, and the resulting traces are used both to challenge and to extend classical M/G/$c$ queueing assumptions.
...
\begin{table}[h]
\centering
\caption{Server implementations used in the experiments. Column $c$ is the number of concurrent workers modelled in the M/G/$c$ DES.}
...
\subsection{Exploratory Priority and Feedback DES}

After the supervisor discussion, two richer scheduling structures were also implemented as exploratory DES variants in \texttt{analysis/des/des\_priority\_exploratory.py}. These are not treated as final models because the existing traces do not contain explicit priority-class labels or feedback-visit identifiers. Their purpose is to test whether a more explicit scheduling interpretation is promising enough to justify new follow-up experiments.

The two exploratory models are:

\begin{itemize}
    \item \textbf{Preemptive-resume priority M/G/1}: observed DSP jobs are treated as low-priority class-2 jobs. A synthetic short class-1 stream is generated as high-priority work. When class-1 work arrives during a DSP job, the DSP job is preempted and later resumes with its remaining service.
    \item \textbf{Feedback priority queue}: each DSP job receives an initial high-priority CPU quantum. If work remains after that quantum, the job feeds back into a low-priority continuation queue.
\end{itemize}

Both models are validated using the same response-CDF and KS-distance procedure as the M0/M1/M2 experiments. The exploratory comparison therefore asks a narrow question: do explicit priority or feedback mechanisms explain the measured DSP response CDF better than the one-stage FCFS model or the M2 load-dependent correction? The answer is documented in Section~\ref{sec:priority-exploratory}: these models improve over plain FCFS M0 in some selected traces but do not outperform M2 overall.
