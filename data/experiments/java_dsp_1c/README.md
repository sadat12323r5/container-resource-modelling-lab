...
| DES model | M/G/4 — `analysis/des/multi_server_des.py --workers 4` |
...
python analysis/load/dsp_aes_load.py --url http://localhost:8086/process --rate 100 --duration 90

# 3. Run DES on the collected trace
python analysis/des/multi_server_des.py --workers 4 \
  --input data/experiments/java_dsp_1c/<trace_file>.csv \
  --mode replay --output des_out.csv

# 4. Re-run all DES modes and regenerate summary + CDF
python analysis/orchestration/run_des_all.py --servers java_dsp
python analysis/reporting/plot_all_cdfs.py java_dsp_1c
```
