...
python analysis/load/sqlite_load.py --url http://localhost:8087/process --rate 25 --duration 90

# 3. Run DES on the collected trace
python analysis/des/single_server_des.py \
  --input data/experiments/go_sqlite_1c/<trace_file>.csv \
  --mode replay --output des_out.csv

# 4. Re-run all DES modes and regenerate summary + CDF
python analysis/orchestration/run_des_all.py --servers go_sqlite
python analysis/reporting/plot_all_cdfs.py go_sqlite_1c
```
