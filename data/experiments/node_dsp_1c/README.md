...
python analysis/load/dsp_aes_load.py --url http://localhost:8084/process --rate 200 --duration 90

# 3. Run DES on the collected trace
python analysis/des/single_server_des.py \
  --input data/experiments/node_dsp_1c/<trace_file>.csv \
  --mode replay --output des_out.csv

# 4. Re-run all DES modes and regenerate summary + CDF
python analysis/orchestration/run_des_all.py --servers node_dsp
python analysis/reporting/plot_all_cdfs.py node_dsp_1c
```
